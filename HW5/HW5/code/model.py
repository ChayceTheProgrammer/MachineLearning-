...
class CSABlock(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        k = self.key(x).view(B, L, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, L, hs)
        q = self.query(x).view(B, L, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, L, hs)
        v = self.value(x).view(B, L, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, L, hs)
        ...
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))  # (B, nh, L, L)

        # Apply causal mask to ensure we can only attend to previous tokens
        # This is the key part that makes the attention "causal"
        att = att.masked_fill(self.mask[:, :, :L, :L] == 0, float('-inf'))

        # attention dropout
        att = F.softmax(att, dim=-1)  # softmax over the last dimension (sequence length)
        attn_save = att.clone()  # save for visualization
